...
RoadMesh RoadNetwork::buildRoadMesh() const {
    ...
    for (const RoadWay& way : ways_) {
        ...
        for (std::size_t i = 0; i + 1 < way.nodes.size(); ++i) {
            ...
            mesh.vertices.push_back({glm::vec3(a.x + normal2.x * halfWidth, 0.0f, a.y + normal2.y * halfWidth)});
            mesh.vertices.push_back({glm::vec3(a.x - normal2.x * halfWidth, 0.0f, a.y - normal2.y * halfWidth)});
            mesh.vertices.push_back({glm::vec3(b.x - normal2.x * halfWidth, 0.0f, b.y - normal2.y * halfWidth)});
            mesh.vertices.push_back({glm::vec3(b.x + normal2.x * halfWidth, 0.0f, b.y + normal2.y * halfWidth)});
